...
import open.pay.center.core.util.BaseHttpSSLSocketFactory;
import open.pay.center.core.util.LogUtil;
import open.pay.center.core.util.StringUtil;
...
public class HttpClient {
	...
	public int send(Map<String, String> data, String encoding) throws Exception {
		try {
			...
			LogUtil.writeLog("Request Message:[" + this.id + "::"+ this.url + "::" + sendData + "]");
			this.requestServer(httpURLConnection, sendData,
					encoding);
			this.result = this.response(httpURLConnection, encoding);
            long endtime = System.currentTimeMillis();
            double useSecond = (endtime - startTime) / 1000.0;
			LogUtil.writeLog("Response message:[" + this.id + "::"+ useSecond + "::" + result + "]");
     ...
     * @throws Exception
     */
    public int send(Param param, String encoding) throws Exception{
        return this.send(param.getData(),encoding);
    }
	...
	public int sendGet(String encoding) throws Exception {
		try {
            ...
            LogUtil.writeLog("Request Message:[" + this.id + "::"+ this.url + "]");
			this.result = this.response(httpURLConnection, encoding);
            long endtime = System.currentTimeMillis();
            double useSecond = (endtime - startTime) / 1000.0;
			LogUtil.writeLog("Response message:[" + this.id + "::"+ useSecond + "::" + result + "]");
	...
	private HttpURLConnection createConnection(String encoding) throws ProtocolException {
		...
		if ("https".equalsIgnoreCase(url.getProtocol())) {
			...
			//TODO 是否验证https证书，测试环境请设置false，生产环境建议优先尝试true，不行再false
